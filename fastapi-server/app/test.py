...
import re
DATABASE='/home/kali/Desktop/ModSercurityPaser/fastapi-server/app/test.db'
...
def send_to_db(cursor , remote_address, remote_port, local_address, local_port, request, time, msg, message):
    try:
        cursor.execute('''
            INSERT INTO MODSECLOG (remote_address, remote_port, local_address, local_port, request, time, msg, message)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (remote_address, remote_port, local_address, local_port, request, time, msg, message))
        conn.commit()
...
def get_msg_from_message(message):
    pattern = r'\[msg "(.*?)"\]'

    # Sử dụng re.findall để tìm tất cả các kết quả phù hợp với mẫu
    matches = re.findall(pattern, message)

    # In thông tin trích xuất
    for match in matches:
        return match.strip()
    return "Not mesage found"        
...
def printModSecLog(modsec_entries):
    """_summary_

    Args:
        modsec_entries (_type_): _description_
    """
    global conn
    cursor= conn.cursor()
    for entry in modsec_entries:
        remote_address = entry['transaction'].get('remote_address', 'None')
        remote_port= entry['transaction'].get('remote_port', 'None')
        local_address= entry['transaction'].get('local_address', 'None')
        local_port= entry['transaction'].get('local_port', 'None')
        request_line= entry['request'].get('request_line', 'None')
        time_event= entry['transaction']['time'].split(" -")[0]
        time_event = convert_to_datetime(time_event)
        list_of_messages = entry['audit_data'].get('messages', [])
        list_error_messages = entry['audit_data'].get('error_messages', [])

        # print data
        print('----------------------------------------')
        print(f'IP: {remote_address}:{remote_port} -> {local_address}:{local_port}')
        print(f'Request: {request_line}')
        print(f'Time: {time_event}')
        print(f'Messages:')
        for message in list_of_messages:
            #print(f'\t{message}') #Message: Warning. Operator GE matched 5 at TX:inbound_anomaly_score. [file "/usr/share/modsecurity-crs/rules/RESPONSE-980-CORRELATION.conf"] [line "92"] [id "980130"] [msg "Inbound Anomaly Score Exceeded (Total Inbound Score: 5 - SQLI=0,XSS=0,RFI=0,LFI=0,RCE=5,PHPI=0,HTTP=0,SESS=0): individual paranoia level scores: 5, 0, 0, 0"] [ver "OWASP_CRS/3.3.5"] [tag "event-correlation"]
            msg= get_msg_from_message(message)
            print(f"\t{msg}")
            send_to_db(cursor,remote_address,remote_port,local_address,local_port,request_line,time_event,msg,str(message))

        print(f'Error Messages:')
        for message in list_error_messages:
            # print(f'\t{message}')
            msg= get_msg_from_message(message)
            print(f"\t{msg}")
            send_to_db(cursor,remote_address,remote_port,local_address,local_port,request_line,time_event,msg,str(message))
...
def save_to_json(modsec_entries,input_filename):
    """_summary_
    ...
    """
    # save to json
    folder_save=f'{input_filename.strip(".log")}'+"_folder/"
    if not os.path.exists(folder_save):
        os.makedirs(folder_save)
    json_output_filename = folder_save + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + '.json'
    modsec_save_json(modsec_entries, json_output_filename, False)
    print(f'----- modsec_audit events saved to: {json_output_filename} -----')

def get_parser(input_filename):
    RECORDS_PROCESSED_CNT=0
    RECORDS_SKIPPED_CNT=0
    RECORDS_TOTAL=0
...
def count_line(file_path):
    try:
        ...
        print(f'Error {e} found during read file {file_path}')
        return None
    
def write_to_file_from_file(path_src,path_dst,index_start):
...
if __name__ == "__main__":
    ...
    input_filename = '/var/log/apache2/modsec/audit.log'
    ...
    while True:
        number_lines=count_line(input_filename)
        if number_lines is None or number_lines < 1:
            time.sleep(10)
            continue

        if NUMBER_LINE_READ < number_lines:
            write_to_file_from_file(input_filename,input_filename_temp,NUMBER_LINE_READ)
            modsec_entries=get_parser(input_filename_temp)
            if len(modsec_entries) < 1:
                print('ERROR : modsec_audit entries to analyze not found with used filters')
            else:
                # save_to_json(modsec_entries,input_filename)
                printModSecLog(modsec_entries)
            NUMBER_LINE_READ=number_lines
